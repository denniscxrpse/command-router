...
from pkg.lib.control import ControlInitialization, ControlResult
from pkg.lib.control.api import *
from pkg.lib.grammar.loader import *
from pkg.sdk import Fixtures
from pkg.suggestions import *
from pkg.suite import *
from pkg.utils import LazyServer, Status, flags, log, paths, stat, uctx
...
class _Router:
    ...
    def genesis_source(self) -> Path | None:
        ...
        if not genesis.exists():
            log.critical("genesis given, but directory %s does not exist", genesis)
            return None
        if not (genesis / "__init__.py").exists():
            log.critical("genesis given, but the existing directory does not contain its `__init__.py` file")
            return None
        return genesis
...
class CommandRouter:
    ...
    def initialize(self) -> Status:
        ...
        grammar_source = paths.FIXTURES if genesis is None else Path(genesis)
        try:
