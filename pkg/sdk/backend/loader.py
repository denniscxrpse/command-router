...
This is the ``pkg.sdk`` entry point for the source-resolution step.
For module names, file paths, and already-imported modules it delegates to
the long-standing loader in ``pkg.lib.control.fixture_loader`` so
...
from pkg.lib.control.fixture_loader import _load_fixture_module as _legacy_load
...
def load_fixture_module(source: ModuleType | str | Path, identifier: int) -> ModuleType:
    ...
    unchanged; see ``pkg.lib.control.fixture_loader`` for the full
    contract.
