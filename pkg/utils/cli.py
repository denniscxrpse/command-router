...
class EnvFlags:
    ...
    `./fixtures/__init__.py` is configured. **WE DO NOT** reconfigure anything in our ``pkg.__init__()``
...
def init_flags(**kwargs) -> None:
    ...
    for key, value in kwargs.items():
        ...
        if value is not None and hasattr(flags, key):
            ...
            if key == "genesis" and value is not None and not isinstance(value, Path):
