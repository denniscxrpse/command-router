...
This is the ``src.sdk`` entry point for the source-resolution step.
For module names, file paths, and already-imported modules it delegates to
the long-standing loader in ``src.lib.control.fixture_loader`` so
...
from command_router.lib.control.fixture_loader import _load_fixture_module as _legacy_load
...
def load_fixture_module(source: ModuleType | str | Path, identifier: int) -> ModuleType:
    ...
    unchanged; see ``src.lib.control.fixture_loader`` for the full
    contract.
