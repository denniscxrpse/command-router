...
syntax; `__init__.py` supplies the Python state and actions used by the control layer. `grammars.py`
is the advanced
...
The control API imports `__init__.py` and looks for one `Fixtures` class derived from
...
from src.sdk import FixturesSDK
...
```python
from example.fixtures import Fixtures
